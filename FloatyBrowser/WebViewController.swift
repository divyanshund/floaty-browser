...
class WebViewController: NSViewController {
    ...
    private var toolbarColorOverlay: NSView?  // Colored overlay for theme colors
    ...
    // MARK: - Theme Color Extraction
    
    private func extractAndApplyThemeColor() {
        // JavaScript to extract theme-color meta tag from the page
        let script = """
        (function() {
            var metaTags = document.getElementsByTagName('meta');
            for (var i = 0; i < metaTags.length; i++) {
                var name = metaTags[i].getAttribute('name');
                if (name && name.toLowerCase() === 'theme-color') {
        ...
        webView.evaluateJavaScript(script) { [weak self] result, error in
            ...
            if let error = error {
                NSLog("❌ Theme color extraction error: \(error.localizedDescription)")
                self.applyDefaultToolbarColor()
                return
            }
            
            if let colorString = result as? String {
                NSLog("🎨 Found theme-color: \(colorString)")
                if let color = self.parseColorString(colorString) {
                    self.applyToolbarColor(color)
                } else {
                    NSLog("⚠️ Could not parse theme-color, using default")
                    self.applyDefaultToolbarColor()
                }
            } else {
                NSLog("ℹ️ No theme-color found, using default")
                self.applyDefaultToolbarColor()
            }
        }
    }
    
    private func parseColorString(_ colorString: String) -> NSColor? {
    ...
    private func parseHexColor(_ hex: String) -> NSColor? {
        ...
        if hexString.count == 3 {
            let r = String(hexString.prefix(1))
            let g = String(hexString.dropFirst().prefix(1))
            let b = String(hexString.dropFirst(2))
    ...
    private func parseRGBColor(_ rgb: String) -> NSColor? {
    ...
    private func applyToolbarColor(_ color: NSColor) {
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            
            // Create or update the color overlay
            if self.toolbarColorOverlay == nil {
                let overlay = NSView(frame: self.toolbar.bounds)
                overlay.wantsLayer = true
                overlay.autoresizingMask = [.width, .height]
                self.toolbarColorOverlay = overlay
                
                // Add overlay below all buttons but above the vibrancy effect
                self.toolbar.addSubview(overlay, positioned: .below, relativeTo: self.toolbar.subviews.first)
            }
            ...
            NSAnimationContext.runAnimationGroup({ context in
                ...
                // Apply color to the overlay with transparency
                self.toolbarColorOverlay?.layer?.backgroundColor = color.withAlphaComponent(0.9).cgColor
                
                // Adjust text color for contrast
                let textColor = self.getContrastColor(for: color)
                self.urlField.textColor = textColor
                self.backButton.contentTintColor = textColor
                self.forwardButton.contentTintColor = textColor
                self.reloadButton.contentTintColor = textColor
                self.newBubbleButton.contentTintColor = textColor
            })
            
            NSLog("✅ Applied theme color to toolbar: \(color)")
        }
    }
    
    private func applyDefaultToolbarColor() {
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            ...
            NSAnimationContext.runAnimationGroup({ context in
                ...
                // Fade out and remove the color overlay to show vibrancy
                if let overlay = self.toolbarColorOverlay {
                    overlay.animator().alphaValue = 0
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                        overlay.removeFromSuperview()
                        self.toolbarColorOverlay = nil
                    }
                }
                
                // Reset text colors to default
                self.urlField.textColor = .labelColor
                self.backButton.contentTintColor = .controlTextColor
                self.forwardButton.contentTintColor = .controlTextColor
                self.reloadButton.contentTintColor = .controlTextColor
                self.newBubbleButton.contentTintColor = .controlTextColor
            })
            
            NSLog("ℹ️ Applied default toolbar style (vibrancy)")
        }
    }
    
    private func getContrastColor(for backgroundColor: NSColor) -> NSColor {
...
extension WebViewController: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        // Extract and apply theme color from website
